...
(function () {
	...
	var RootOf, SpawnFrom, PassRootIntoMethod, RandomIntMethod, NewUniqueIdMethod;
	...
	PassRootIntoMethod = function passRootInto(rootExtensionAction_) { 
	...
	function NewConstructor(selector, parentConstructor) {
		...
		constructor.passRootInto = PassRootIntoMethod;
	...
	window.Top = new Module("Top", window, function () {
		...
		this.addMethod(function newGUID() { return NewGUID(GUID_SEGMENTS.map(RandomIntMethod)); });
		...
		this.addMethod(function isTopObject(target) {
			switch (typeof target) {
				case "function" : return (target.passRootInto === PassRootIntoMethod);
